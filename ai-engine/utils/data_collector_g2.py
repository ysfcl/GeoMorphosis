"""
GeoMorphosis - Ic Anadolu Bolgesi Egitim Veri Seti Toplayici (YOLOv8 Uyumlu)
GEE ile Sentinel-2 goruntulerini indirir, Ic Anadolu bolgelerini Grid'lere bolerek indirir.
Her yil icin yaklasik ~200 adet 640x640 boyutunda karo (tile) indirir.

Kullanim:
    python utils/data_collector_g2.py --region eskisehir --years 2021 2022 2023 2024 2025
    python utils/data_collector_g2.py --region konya --years 2021 2022 2023 2024 2025
"""
...
REGIONS = {
    "eskisehir": {
        ...
        "bbox": [30.25, 39.55, 30.85, 40.05],
        "description": "Ic Anadolu - Eskisehir (Porsuk Cayi, Tarim ve Sanayi Alani)",
    },
    "konya": {
        "name": "Konya",
        "bbox": [32.30, 37.60, 32.90, 38.10],
        "description": "Ic Anadolu - Konya (Kuraklik, Tarim ve Gol Cekilmeleri)",
    },
    "ankara": {
        "name": "Ankara",
        "bbox": [32.50, 39.70, 33.10, 40.20],
        "description": "Ic Anadolu - Ankara ve Cevresi",
    }
}
...
def generate_grid(bbox, grid_size_deg=0.055):
    """
    Genis BBOX alanini kucuk karelere (Grid) boler.
    grid_size_deg=0.055 ayari ile yillik toplam ~200 gorsel indirilecek sekilde ayarlanmistir.
    """
...
def collect_region_dataset(region_name="eskisehir", years=None):
    init_gee()

    if years is None:
        years = DEFAULT_YEARS

    region = REGIONS.get(region_name, REGIONS["eskisehir"])
    ...
    # Yıllık ~200 görsel hedeflendiği için 3 farklı ay (3 x ~67 = ~200 tile/yıl)
    monthly_periods = [
        ("06-01", "06-30", "jun"),
        ("07-01", "07-31", "jul"),
        ("08-01", "08-31", "aug")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="GEE Veri Toplayıcı (İç Anadolu)")
    parser.add_argument("--region", default="eskisehir", choices=list(REGIONS.keys()))
